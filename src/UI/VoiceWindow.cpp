    ...
    : QWidget(parent), userId(userId)
{
    ...
    connect(&speakingDecayTimer, &QTimer::timeout, this, [this]() {
        ...
        if (speakingGlowAnim)
            speakingGlowAnim->stop();
        if (speakingGlowFx)
            speakingGlowFx->setOpacity(1.0);
    });
...
void VoiceUserWidget::setSpeaking(bool speaking)
{
    if (speaking == isCurrentlySpeaking) {
        // still update the avatar if we got a direct speaking call
        if (speaking)
    ...
    if (speaking) {
        ...
        if (!speakingGlowFx) {
            ...
            speakingGlowFx->setOpacity(1.0);
            setGraphicsEffect(speakingGlowFx);
        }
        if (!speakingGlowAnim) {
            ...
            speakingGlowAnim->setLoopCount(-1);
        }
        speakingGlowAnim->start();
        speakingDecayTimer.start();
    } else {
        speakingDecayTimer.stop();
        avatarWidget->setSpeaking(false);

        // Stop glow and restore
        if (speakingGlowAnim)
            speakingGlowAnim->stop();
        if (speakingGlowFx)
            speakingGlowFx->setOpacity(1.0);
...
void VoiceWindow::setupUi()
{
    ...
    connect(inputGainSlider, &QSlider::valueChanged, this, [this](int value) {
        ...
        QSettings().setValue("voice/inputGain", value);
        if (voiceManager)
    ...
    connect(outputVolumeSlider, &QSlider::valueChanged, this, [this](int value) {
        ...
        QSettings().setValue("voice/outputVolume", value);
        if (voiceManager)
    ...
    connect(vadThresholdSlider, &QSlider::valueChanged, this, [this](int value) {
        ...
        QSettings().setValue("voice/vadThreshold", value);
        volumeMeter->setThreshold(static_cast<float>(value));
        if (voiceManager)
            voiceManager->setVadThreshold(static_cast<float>(value));
    });

    // Restore persisted values so the window reflects prior adjustments.
    inputGainSlider->setValue(QSettings().value("voice/inputGain", 100).toInt());
    outputVolumeSlider->setValue(QSettings().value("voice/outputVolume", 100).toInt());
    vadThresholdSlider->setValue(QSettings().value("voice/vadThreshold", 100).toInt());
...
void VoiceWindow::onParticipantJoined(Core::Snowflake userId)
{
    ...
    // Restore the persisted per-user volume so the slider shows what the user
    ...
    connect(widget, &VoiceUserWidget::volumeChanged, this, [this](Core::Snowflake uid, int pct) {
        ...
        QSettings().setValue("voice/volume/" + QString::number(uid), pct);
        if (voiceManager)
            voiceManager->setUserVolume(uid, qBound(0.0f, static_cast<float>(pct) / 100.0f, 2.0f));
    });
    ...
    Core::AnimationUtils::fadeIn(widget, 200);
...
void VoiceWindow::onParticipantLeft(Core::Snowflake userId)
{
    ...
    QObject::connect(fade, &QPropertyAnimation::finished, w, &QObject::deleteLater);
