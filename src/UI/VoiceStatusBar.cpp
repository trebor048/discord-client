...
VoiceStatusBar::VoiceStatusBar(QWidget *parent) : QWidget(parent)
{
    ...
    auto *dotFx = new QGraphicsOpacityEffect(statusDot);
...
void VoiceStatusBar::updateConnectionState()
{
    ...
    if (active) {
        if (!isVisible())
            setVisible(true);
        // Pulse the status dot while connected — don't use AnimationUtils::fadeIn/fadeOut
        // here because stopExistingOpacityAnimations recursively kills the pulse animation.
        if (connected && pulseAnim)
            pulseAnim->start();
        else if (pulseAnim)
            pulseAnim->stop();
    } else if (isVisible()) {
...
{
    if (!voiceWindow) {
        voiceWindow = new VoiceWindow(this);
        configureVoiceWindow();
    }
