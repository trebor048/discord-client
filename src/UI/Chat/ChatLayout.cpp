...
QString buildRichTextStyleSheet()
{
    ...
    // Fenced code blocks: monospace plus per-token colors driven by the theme.
    const QColor codeKeyword = Manager::instance().color(Token::LinkText);
    const QColor codeString = Manager::instance().color(Token::BrightText);
    const QColor codeComment = Manager::instance().color(Token::PlaceholderText);
    const QColor codeNumber = Manager::instance().color(Token::Highlight);

    QString codeBlock = QStringLiteral(".code-block { font-family: '%1';").arg(codeFont.family());
    if (codeFont.pointSizeF() > 0)
        codeBlock += QStringLiteral(" font-size: %1pt;").arg(codeFont.pointSizeF());
    codeBlock += QStringLiteral(" } ");
    codeBlock += QStringLiteral(".code-kw { color: %1; }").arg(codeKeyword.name(QColor::HexRgb));
    codeBlock += QStringLiteral(".code-str { color: %1; }").arg(codeString.name(QColor::HexRgb));
...
int calculateAttachmentsHeight(const QList<AttachmentData> &attachments, int textWidth)
{
    ...
    totalHeight += videoCount * (videoAttachmentHeight() + padding());
...
MessageLayout calculateMessageLayout(const LayoutContext &ctx)
{
    ...
    const int leftGutter = ctx.compactMode ? padding() : padding() + avatarSize() + padding();
    const int topPadding = ctx.compactMode ? padding() : blockTopPadding();

    int textLeft = leftGutter;
    ...
    if (layout.hasReply) {
        ...
        int avatarTop = ctx.rowTop + topPadding + separatorOffset;
    ...
    if (layout.hasReply) {
        ...
        if (ctx.showHeader)
            textTop += topPadding - capDrop + fm.height();
        else
            textTop += 0;
    }
    ...
    if (layout.hasReply) {
        ...
        int contentHeight = topPadding - capDrop + fm.height() + layout.textHeight + padding() / 2;
        totalHeight = contentHeight;
    } else {
        totalHeight = layout.textHeight + (ctx.compactMode ? 0 : padding() / 2);
    }
    ...
    if (!ctx.attachments.isEmpty()) {
        ...
        for (int i = 0; i < ctx.attachments.size(); ++i) {
            ...
            if (ctx.attachments[i].isVideo) {
                // Video attachments get a larger, thumbnail-like tile.
                attLayout.rect =
                        QRect(textLeft, currentFileTop, fileWidth, videoAttachmentHeight());
                layout.videoLayouts.append(attLayout);
            } else {
                attLayout.rect = QRect(textLeft, currentFileTop, fileWidth, fileAttachmentHeight());
                layout.fileLayouts.append(attLayout);
            }

            currentFileTop += attLayout.rect.height() + padding();
            layout.attachmentsTotalHeight += attLayout.rect.height() + padding();
            fileIndex++;
        }
    ...
    if (!ctx.stickers.isEmpty()) {
        ...
        int currentY = layout.stickersTop + padding();
        ...
        for (int i = 0; i < ctx.stickers.size(); ++i) {
            ...
            QRect sr(currentX, currentY, stickerSize(), stickerSize());
            layout.stickerRects.append(sr);
            currentX += stickerSize() + stickerSpacing();
        }

        if (!ctx.stickers.isEmpty())
            layout.stickersTotalHeight = padding()
                + (currentY - layout.stickersTop - padding()) + stickerSize();
        totalHeight += layout.stickersTotalHeight;
    }

    layout.reactionsTop = layout.stickersTop + layout.stickersTotalHeight;
    ...
    layout.quickReaction = {};
    if (!ctx.hasSeparator && !ctx.isSystemMessage) {
        const QStringList &emojis = quickReactionEmojis();
        const int buttonCount = emojis.size() + 1; // + "more" button
        const int barWidth = quickReactionBarPadding() * 2 +
                             buttonCount * quickReactionButtonSize() +
                             (buttonCount - 1) * quickReactionButtonSpacing();
        const int barRight = ctx.rowWidth - quickReactionBarMargin();
        const int barLeft = qMax(0, barRight - barWidth);

        int centerY;
        if (layout.showHeader)
            centerY = layout.headerRect.center().y();
        else
            centerY = layout.textRect.top() + fm.height() / 2;
        const int barTop = centerY - quickReactionBarHeight() / 2;

        layout.quickReaction.barRect =
                QRect(barLeft, barTop, qMin(barWidth, ctx.rowWidth), quickReactionBarHeight());

        int x = barLeft + quickReactionBarPadding();
        const int y = barTop + (quickReactionBarHeight() - quickReactionButtonSize()) / 2;
        for (int i = 0; i < emojis.size(); ++i) {
            layout.quickReaction.buttonRects.append(
                    QRect(x, y, quickReactionButtonSize(), quickReactionButtonSize()));
            x += quickReactionButtonSize() + quickReactionButtonSpacing();
        }
        layout.quickReaction.moreButtonRect =
                QRect(x, y, quickReactionButtonSize(), quickReactionButtonSize());
    }
    ...
    for (int ei = 0; ei < layout.embedLayouts.size(); ++ei) {
        ...
        if (embed.images.isEmpty() && !embed.videoThumbnail.isNull() && embed.thumbnail.isNull() && !el.imagesRect.isNull())
            layout.hitRegions.append({ HitRegion::Kind::EmbedVideoThumbnail, el.imagesRect, ei, -1, embed.videoUrl.toString() });
    ...
    for (const auto &al : layout.videoLayouts)
        layout.hitRegions.append({ HitRegion::Kind::AttachmentVideo, al.rect, al.index, -1, {} });
...
ResolvedLayout resolveLayout(const QAbstractItemView *view, const QModelIndex &index)
{
    ...
    result.ctx = buildContext(view, index, view->font(), view->visualRect(index), view->palette());
