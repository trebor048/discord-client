...
{
    // Clamp a 0/missing interval so the heartbeat loop doesn't busy-spin.
    int interval = obj["heartbeat_interval"].toInt();
    if (interval <= 0)
        interval = 41250;
        ...
        QMetaObject::invokeMethod(
                ...
                [self, ok, httpCode, response, ticket, attempt]() {
                    ...
                    if (!ok) {
                        ...
                        if (challenge && self->captchaResolver && attempt < kMaxCaptchaAttempts) {
                            self->captchaResolver->resolve(
                                    *challenge,
                                    [self, ticket, attempt](std::optional<CaptchaSolution> sol) {
                                        if (!self || self->done)
                                            return;
                                        if (!sol) {
                                            self->fail(RemoteAuthError::CaptchaRequired);
                                            return;
                                        }
                                        self->postLogin(ticket, sol, attempt + 1);
                                    });
                            return;
                        }

                        const bool captcha = challenge.has_value();
                        if (attempt > 0 && self->captchaResolver)
                            self->captchaResolver->notifyConcluded();
                        qCWarning(LogNetwork) << "Remote auth login failed, HTTP" << httpCode << (captcha ? "(captcha required)" : "");
                        self->fail(captcha ? RemoteAuthError::CaptchaRequired
