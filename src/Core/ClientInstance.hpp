...
public:
    ...
    [[nodiscard]] const QHash<Snowflake, QList<Discord::Sticker>> &guildStickers() const { return stickerStore; }
#ifndef ACHERON_NO_VOICE
...
private:
    ...
    bool saveGuild(const Discord::GatewayGuild &guild, const QList<Discord::Member> *members, Snowflake myId, QSqlDatabase &db);
    void initGuildReadState(const Discord::GatewayGuild &guild);
    void cleanupPendingMemberRequests();
    ...
    QTimer *pendingMemberRequestCleanupTimer = nullptr;

    QHash<Snowflake, QList<Discord::Sticker>> stickerStore;
