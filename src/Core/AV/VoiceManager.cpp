...
void VoiceManager::setPushToTalkEnabled(bool enabled)
{
    if (cachedPushToTalk == enabled)
        return;

    cachedPushToTalk = enabled;

    if (audioPipeline)
        QMetaObject::invokeMethod(audioPipeline, [p = audioPipeline, enabled]() { p->setPushToTalkEnabled(enabled); });

    // Disabling PTT must never leave the mic gated by a stale held state.
    if (!enabled)
        setPushToTalkKeyHeld(false);
}

void VoiceManager::setPushToTalkKeyHeld(bool held)
{
    if (pushToTalkKeyHeld == held)
        return;

    pushToTalkKeyHeld = held;

    if (audioPipeline)
        QMetaObject::invokeMethod(audioPipeline, [p = audioPipeline, held]() { p->setPushToTalkKeyHeld(held); });
}
...
void VoiceManager::onVoiceClientConnected()
{
    ...
    QMetaObject::invokeMethod(audioPipeline, [p = audioPipeline, backend, capturing, inputId, outputId,
        ...
        p->setPushToTalkEnabled(pttEnabled);
        p->setPushToTalkKeyHeld(pttHeld);
