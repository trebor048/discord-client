...
void RoleRepository::bindRole(QSqlQuery &q, Core::Snowflake guildId, const Discord::Role &role)
{
    ...
    q.bindValue(":permissions", static_cast<qint64>(role.permissions.get().toInt()));
...
bool RoleRepository::saveRole(Core::Snowflake guildId, const Discord::Role &role, QSqlDatabase &db)
{
    QSqlQuery q(db);
    q.prepare(ROLE_UPSERT_SQL);
    bindRole(q, guildId, role);
    return execLogged(q, "RoleRepository: Save role");
}

bool RoleRepository::saveRoles(Core::Snowflake guildId, const QList<Discord::Role> &roles,
                               QSqlDatabase &db)
{
    if (roles.isEmpty())
        return true;
