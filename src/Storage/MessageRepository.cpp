    ...
    for (const auto &message : messages) {
        ...
        if (message.attachments.hasValue()) {
            for (const auto &att : *message.attachments) {
                ...
                if (!execLogged(qAtt, "MessageRepository: Save attachment"))
                    goto rollback;
            }
        }
    }
...
std::optional<Discord::Message> MessageRepository::getMessage(Core::Snowflake messageId)
{
    ...
    if (!execLogged(q, "MessageRepository: Get message"))
        return std::nullopt;
    ...
    QList<Discord::Message> list{ message };
    loadAttachmentsForMessages(list, db);
...
Discord::Message MessageRepository::readMessageFromQuery(const QSqlQuery &q)
{
    ...
    if (q.value(10).isNull()) {
        // User was deleted from cache — LEFT JOIN returned NULLs
        message.author->id = static_cast<Core::Snowflake>(q.value(2).toLongLong());
        message.author->username = QStringLiteral("Unknown User");
        message.author->globalName = QStringLiteral("Unknown User");
    } else {
    ...
    if (!q.value(15).isNull()) {
        ...
        if (!q.value(16).isNull()) {
            ...
            if (!q.value(25).isNull()) {
                ...
                // Referenced message author was deleted from cache
                refMsg->author->id = static_cast<Core::Snowflake>(q.value(18).toLongLong());
                refMsg->author->username = QStringLiteral("Unknown User");
                refMsg->author->globalName = QStringLiteral("Unknown User");
            }
